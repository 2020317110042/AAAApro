...
public class CPFModelParser {
    ...
    /*This function is same the above function 'readModels', the difference is that the disconnected canonical net can be
    * transformed into a single graph. In some scenarios, when edge is removed from a process model, we need this function.*/
    public static Graph readModels2SingleGraph(CanonicalProcessType cpf) {
        Graph epcGraph = new Graph();
        ...
        for (Vertex v : g.getVertices()) {
            ...
            if (v.getType().equals(Vertex.Type.state)) {
                ...
                if (v.isAddedGW()) {
                    ...
                    /*added by Zaiwen on 9 June 2016*/
                    if (v.getGWType().equals(GWType.and)) {
                        n = new ANDSplitType();
                    } else if (v.getGWType().equals(GWType.or)) {
                        n = new ORSplitType();
