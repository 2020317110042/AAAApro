...
public class ManagerPortalEndpoint {
    ...
    @ResponsePayload
    public JAXBElement<SearchForSimilarProcessesOutputMsgType> searchForSimilarProcesses(@RequestPayload final JAXBElement<SearchForSimilarProcessesInputMsgType> req) {
    ...
    public JAXBElement<UpdateProcessOutputMsgType> updateProcess(@RequestPayload final JAXBElement<UpdateProcessInputMsgType> req) {
        ...
        try {
            ...
            //get the index for determining if propagate changes
            ...
            //on the condition that we consider change propagation from G to CG
            if(propagationIndex==1){
                ...
                //为最初的流程变体设置'ProcessVersionIdType'
                ...
                //为改变的流程变体设置'ProcessVersionIdType'
                ...
                //设置合并模型的新分支名称，我们将其设置为与变体的新分支名称相同
                String newBranchNameForMergedModel = editType.getNewBranchName();
                
                // 单位为毫秒
                //设置合并模型的新版本号，我们将其设置为等于变体的新版本
                Version newVersionForMergedModel = currentVersion;

                changePropagationSrv.changePropagationFromGtoCG(username,
                        originalProcessVariant,
                        changedProcessVariant,
                        newBranchNameForMergedModel,
                        newVersionForMergedModel,
                        null);
                        //修改'nativeType'为'null'     by zaiwen 17 Mar 2016

            }

            //on the condition that we consider change propagation from CG to G
            else if(propagationIndex==2){

                //add code for the change propagation function in the following ...
                String username = payload.getEditSession().getUsername();

                //set the 'ProcessVersionIdType' of the original merged model
                ProcessVersionIdType originalMergedVersion = new ProcessVersionIdType();
                originalMergedVersion.setProcessId(editType.getProcessId());
                originalMergedVersion.setBranchName(editType.getOriginalBranchName());
                originalMergedVersion.setVersionNumber(editType.getOriginalVersionNumber());

                //set the 'ProcessVersionIdType' of the changed merged model
                ProcessVersionIdType changedMergedVersion = new ProcessVersionIdType();
                changedMergedVersion.setProcessId(editType.getProcessId());
                changedMergedVersion.setBranchName(editType.getNewBranchName());
                changedMergedVersion.setVersionNumber(editType.getCurrentVersionNumber());

                //set the new branch name of the changed variant, we set it equal to the new branch name of the changed merged model
                String updated_variant_branch_name = editType.getNewBranchName();

                //set the new version number of the changed variant(s), we set it equal to the new version of the changed merged model
                Version updated_variant_version = currentVersion;

                changePropagationSrv.changePropagationFromCGtoG(username,originalMergedVersion,
                        changedMergedVersion,  updated_variant_branch_name, updated_variant_version, null); //modify 'nativeType' to 'null' by zaiwen 17 Mar 2016
